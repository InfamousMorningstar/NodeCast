...
export async function restoreFile(
  req: FastifyRequest<{ Body: RestoreFileRequest }>,
  res: FastifyReply
) {
  try {
    ...
    const result = await zfsVersioning.restoreFileFromSnapshot(
      filePath,
      snapshotName,
      userId
    );
...
export async function compareVersions(
  ...
  try {
    ...
    const result = await zfsVersioning.compareFileVersions(
      filePath,
      fromSnapshot,
      toSnapshot
    );
