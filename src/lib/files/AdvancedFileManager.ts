...
export class AdvancedFileManager {
  ...
  async searchFilesByTag(tag: string, userId: string): Promise<FileMetadata[]> {
    ...
    for (const file of files) {
      ...
      results.push({
        ...
        autoTags: await this.getAutoGeneratedTags(filePath, file.mimetype)
      });
    }
  ...
  private async evaluateConditions(conditions: SmartFolderCondition[], filePath: string, fileStats: any): Promise<boolean> {
  ...
  private async evaluateCondition(condition: SmartFolderCondition, filePath: string, fileStats: any): Promise<boolean> {
    let value: any;
    ...
    for (const file of files) {
      ...
      for (const rule of this.organizationRules) {
        ...
        if (match) {
          ...
          const autoTags = await this.getAutoGeneratedTags(currentPath, file.mimetype);
